	...
	<div class="featured-card-overlay">
		<div class="featured-card-layout">
			<figure class="featured-card-poster">
				<img src={featuredPosterUrl} alt={`${featuredTitle} Poster`} />
			</figure>
			...
			<div class="featured-card-content">
								...
								<dd class="featured-card-genres">
									...
									<ul>
										{#each featuredGenres as genre, index (`featured-genre-${genre.id ?? genre.name}`)}
											<li>
												{#if index > 0}<span class="featured-card-genre-separator" aria-hidden="true">/</span>{/if}<span>{genre.name}</span>
											</li>
										{/each}
									</ul>
								</dd>
				...
				<h2 id="featured-card-title" class="featured-card-title">{featuredTitle}</h2>
...
<style lang="scss">
	.ui.featured-card {
		...
		background-image:
			linear-gradient(to top, rgba(0, 0, 0, 0.92) 0%, rgba(0, 0, 0, 0.7) 42%, rgba(0, 0, 0, 0.35) 100%),
