# Acceptance Tests – Typeahead Search
...
The purpose of this test plan is to validate the typeahead search from a user's perspective. The tests verify that users can search movies and TV shows, navigate from a result to the matching details page, and receive appropriate feedback for loading, empty-result, and network-error states. Keyboard navigation tests (Tab, Escape) are covered separately in accessibility tests.
...
1. Search results are displayed after entering at least four characters.  
2. A selected result opens its matching movie or TV-show detail page.  
3. Detail-page title matches the selected result title.  
4. A TV-show result opens a `/tv-shows/[id]` route.  
5. Empty searches display the no-results status message.  
6. A pending request displays the loading status message.  
7. A failed request displays an alert.  
8. Search results are not rendered for loading, empty-result, or error states.  
9. Clicking outside the header closes the result list.  
...
- Keyboard focus progression (Tab, Arrow keys, Home, End) – covered in accessibility tests.  
- Escape key behaviour – covered in accessibility tests.  
...
- **Tags:** `@search`, `@typeahead`, `@e2e`, `@black-box`, `@regression`, `@negative`, `@boundary`  
...
|-------------------------|-------------------------------------------------|----------------------|-------------------|
| Results available       | `#typeahead-search-results`                     | Visible              | TC‑TS‑001, TC‑TS‑002, TC‑TS‑006 |
| Query < 4 characters    | No results or status layer                      | Not rendered         | TC‑TS‑004         |
...
| Click outside closes    | `#typeahead-search-results` becomes hidden      | Hidden after click   | TC‑TS‑006         |

### Rationale

- The four-character threshold is covered with boundary-value analysis: three characters represent the invalid boundary and four characters the valid boundary. We only test valid boundaries.  
...
- Click-outside behaviour is covered because the component must close when the user clicks elsewhere on the page.  
...
- Browser: Chromium, viewport: 1920×¹⁰⁸⁰  
- Live search is available  

| Step | Action | Expected Result |
|------|--------|-----------------|
| 1 | Navigate to `/?locale=en-US` | Home page is displayed |
| 2 | Clear the search field and enter „Fight Club" | Search request is triggered after the minimum input length |
...
- App running on `localhost:4173`  
- Browser: Chromium, viewport: 1920×¹⁰⁸⁰  
- Live search returns a TV-show result for „Dark"  

| Step | Action | Expected Result |
|------|--------|-----------------|
| 1 | Navigate to `/?locale=en-US` | Home page is displayed |
| 2 | Clear the search field and enter „Dark" | Search request is triggered |
...
### TC-TS-005: Click outside closes results

- **Feature:** F-TS (Typeahead Search)  
- **Module:** Typeahead Search / Pointer interaction  
- **Priority:** Medium  
- **Type:** Functional (Black-Box)  
- **Tags:** `@search`, `@typeahead`, `@e2e`, `@black-box`, `@regression`  
...
- App running on `localhost:4173`  
- Browser: Chromium, viewport: 1920×¹⁰⁸⁰  
- Search term „Fight Club" returns results  

| Step | Action | Expected Result |
|------|--------|-----------------|
| 1 | Navigate to `/?locale=en-US` | Home page is displayed |
| 2 | Clear the search field and enter „Fight Club" | Results layer is visible |
| 3 | Click outside the search area (e.g., on the page body) | Results layer closes |
| 4 | Verify `#typeahead-search-results` | Results layer is hidden or removed |

**Pass/Fail Criteria:**

- Clicking outside the search area closes the currently visible result list.  

---

### TC-TS-006: Loading status during pending request
...
### TC-TS-007: Network-error status
...
Feature: Typeahead Search (F-TS)
  ...
  Scenario: A search result opens the matching details page
    When I clear the search field if necessary and enter "Fight Club"
  ...
  Scenario: A TV-show result opens a TV-show details page
    When I clear the search field if necessary and enter "Dark"
  ...
  @TC-TS-005
  Scenario: Click outside closes search results
    When I clear the search field if necessary and enter "Fight Club"
...
npx playwright test -g "(?=.*@search)(?=.*@boundary)"
...
- **Test Cases:** TC‑TS‑001 through TC‑TS‑007  
- **Dependencies:** Live development server; live search data for result-navigation flows  
- **Controlled requests:** TC‑TS‑006 delays the local search route and TC‑TS‑007 aborts it through Playwright routing  
- **Viewport:** Desktop (1920×¹⁰⁸⁰)  
...
| TC‑TS‑005   | F‑TS    | Click outside         | Functional (Black-Box)      | Medium   | `@search`, `@typeahead`, `@e2e`, `@black-box`, `@regression` |
| TC‑TS‑006   | F‑TS    | Loading state         | State Transition (Black-Box)| Medium   | `@search`, `@typeahead`, `@e2e`, `@black-box`, `@regression` |
| TC‑TS‑007   | F‑TS    | Network error         | Negative / Error Guessing   | High     | `@search`, `@typeahead`, `@e2e`, `@black-box`, `@negative`, `@regression` |