...
test.describe('Load More - Home (F-LM)', () => {
	...
	// -------------------------------------------------------------------------
	test(
		'[TC-LM-001] loads more cards on home page',
		{
	...
	// -------------------------------------------------------------------------
	test(
		'[TC-LM-002] shows loading state during fetch',
		{
			tag: ['@loadmore', '@home', '@black-box', '@regression']
		},
		async ({ page }) => {
	...
	// -------------------------------------------------------------------------
	test(
		'[TC-LM-007] handles empty results',
		{
			tag: ['@loadmore', '@home', '@black-box', '@regression']
		},
		async ({ page }) => {
			...
			await page.waitForResponse(
				(resp) =>
					resp.url().includes('/trending') && resp.url().includes('page=2') && resp.status() === 200
			);
