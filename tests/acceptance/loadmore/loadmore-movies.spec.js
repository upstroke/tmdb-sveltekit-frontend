...
test.describe('Load More - Movies (F-LM)', () => {
	// TC-LM-003
	test(
		'[TC-LM-003] loads more cards on movies page',
		{
		...
		async ({ page }) => {
			...
			await Promise.all([
				page.waitForResponse(
					(resp) =>
						resp.url().includes('/movies') && resp.url().includes('page=') && resp.status() === 200
				),
	...
	// TC-LM-004
	test(
		'[TC-LM-004] handles error response',
		{
			tag: ['@loadmore', '@movies', '@black-box', '@regression']
		},
		async ({ page }) => {
			...
			// Mock the movies API to return a 500 error
			...
			// so the response cannot be missed.
			await Promise.all([
				page.waitForResponse(
					(resp) =>
						resp.url().includes('/movies') && resp.url().includes('page=') && resp.status() === 500
				),
				loadMoreButton.click()
			]);

			// Verify that the button is enabled again after the error
			...
			// Verify that no new cards were loaded
			const finalCount = await cardItems.count();
	...
	// TC-LM-008
	test(
		'[TC-LM-008] handles empty results',
		{
			tag: ['@loadmore', '@movies', '@black-box', '@regression']
		},
		async ({ page }) => {
			...
			await page.waitForResponse(
				(resp) =>
					resp.url().includes('/movies') && resp.url().includes('page=2') && resp.status() === 200
			);
