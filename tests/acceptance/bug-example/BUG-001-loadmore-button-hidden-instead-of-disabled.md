...
### Severity-Matrix
| Level | Severity           | Description                                | Impact                                           | Priority               |
| ----- | ------------------ | ------------------------------------------ | ------------------------------------------------ | ---------------------- |
| S1    | Critical / Blocker | System crash, data loss, complete outage   | Core functionality completely broken             | Immediate fix required |
| S2    | Major / High       | Major feature broken, difficult workaround | Significant functionality impaired               | Fix in current sprint  |
| S3    | Minor / Medium     | Bug present, easy workaround available     | Minor functionality affected, core features work | Fix in next release    |
| S4    | Trivial / Low      | Cosmetic issues, typos, UI inconsistencies | No functional impact                             | Fix when convenient    |
...
### Priority Matrix
| Level | Severity           | Priority    | Description                                | Example                             |
|-------|--------------------|-------------|--------------------------------------------|-------------------------------------|
| S1    | Critical / Blocker | P1 Critical | System crash, data loss, complete outage   | Login broken for all users          |
| S2    | Major / High       | P1 Critical | Major feature broken, difficult workaround | Checkout fails intermittently       |
| S3    | Minor / Medium     | P2 Medium   | Bug present, easy workaround available     | Load More button needs second click |
| S4    | Trivial / Low      | P3 Low      | Cosmetic issues, typos, UI inconsistencies | Wrong color on button               |
...
- **Test Documentation:** `../loadmore/loadmore-testplan.md`
- **Affected Test Cases:**
  - TC-LM-006: `../loadmore/loadmore-tvshows.spec.js`
  - TC-LM-007: `../loadmore/loadmore-home.spec.js`
  - TC-LM-008: `../loadmore/loadmore-movies.spec.js`
...
```
tests/acceptance/bug-example/BUG-001-loadmore-button-hidden-instead-of-disabled.md
```
...
- ✅ The `..` folder contains all related artifacts:
  ...
  - Test plan documentation (`../loadmore/loadmore-testplan.md`)
